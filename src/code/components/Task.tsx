...
const Task: React.FC<Props> = ({ task, status, reset }) => {
  ...
  return (
    <li className="list-group-item border-right-0 border-left-0 p-0 d-flex justify-content-between align-items-center h-100 w-100">
      ...
      <div className="mx-3 my-2 h-100">
        <button className="btn btn-sm btn-primary" onClick={handleChange}>
          {status === "active" ? "Complete" : "Restart"}
        </button>
      </div>
    </li>
  );
